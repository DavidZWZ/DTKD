...
class FMFirstOrderLinear(nn.Module):
    ...
    def __init__(self, config, dataset, output_dim=1):
        ...
        for field_name in self.field_names:
            ...
            if dataset.field2type[field_name] == FeatureType.TOKEN:
                ...
                # change the token_seq_field_dim of title
                if field_name == self.title:
                    self.token_seq_field_dims.append(dataset.field2feats(self.title)[0][self.title].size(1))
                else:    
        ...
        if len(self.token_seq_field_dims) > 0:
            ...
            for token_seq_field_dim in self.token_seq_field_dims:
                if token_seq_field_dim == 384:
                    self.token_seq_embedding_table.append(nn.Linear(token_seq_field_dim, output_dim))
                else:
    ...
    def embed_token_seq_fields(self, token_seq_fields):
        ...
        for i, token_seq_field in enumerate(token_seq_fields):
            ...
            if token_seq_field.size(1) == 384:
                fields_result.append(token_seq_embedding.unsqueeze(2))
            else:
